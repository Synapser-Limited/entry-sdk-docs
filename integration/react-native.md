...
| Android minSdk        | 24 (Android 7.0) — for current Entry SDK releases such as `3.1.51`                     |
...
- **Keeping Android minSdk below the SDK requirement** — `com.synapser:entry-sdk:3.1.51` requires `minSdk = 24`; an app with `minSdk = 21` will fail to build
