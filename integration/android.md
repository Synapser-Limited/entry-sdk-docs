...
| Kotlin         | 1.7+                                                                      |
| Gradle         | 7.0+                                                                      |
| minSdk         | 24 (Android 7.0) for current Entry SDK releases such as `3.1.49`          |
| Device         | Physical Android device with front camera — the emulator is not supported |

> If your app sets `minSdk` lower than the Entry SDK's declared minimum, Gradle will fail manifest merging. For example, `com.synapser:entry-sdk:3.1.49` requires `minSdk 24`.
...
```text
uses-sdk:minSdkVersion 21 cannot be smaller than version 24 declared in library [com.synapser:entry-sdk:3.1.49]
```
...
- **Do not keep `minSdk` below the SDK requirement** — for example, `com.synapser:entry-sdk:3.1.49` requires `minSdk = 24`, so an app with `minSdk = 21` will fail to build
